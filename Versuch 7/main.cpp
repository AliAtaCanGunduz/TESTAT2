...
/*
* @brief Die Funktion, die das Einlesen der Datei ermoeglicht.
...
int main()
{
    ...
    do
    {
        std::cout << "\nMenue:" << std::endl
            ...
            << "(0): Beenden" << std::endl << "(4) : Datenbank rueckwaertsausgeben" << std::endl << "(5):Datenelement vorne loeschen" << std::endl << "(6):Datenelement vorne hinzufuegen" << std::endl << "(R):Daten aus einer Datei einlesen" << std::endl << "(W):Daten in eine Datei sichern" << std::endl << "(S):Daten Sortieren" << std::endl << "(F):Daten Finden" << std::endl;
        std::cin >> abfrage;
        ...
        switch (abfrage)
        {
            ...
            case 'S':
            {
                std::sort(liste.begin(), liste.end());//basic sorting
            }
            break;
            case 'F':
            {
                Student student(64567, "Paula Peters", "9.01.1981", "Weidenweg 12");
                std::vector<Student>::iterator it = std::find(liste.begin(), liste.end(), student);//iterator to find student
                std::cout << *it << "wird geloescht \n \n \n" << std::endl;
                liste.erase(it);
